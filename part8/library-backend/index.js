...
`
// allAuthors: [Author!] -> I am defining that allAuthors will have an array of Authors.
// from here it know to execute bookCount for each Author element inside the array.
...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      //   if (genre) {
      //     const genredBooks = authorBooks.filter((book) => book.genres.find((g) => g === genre))
      //     return genredBooks
      //   }
      //   return authorBooks
      // }
      // else if(genre) {
      //   const genredBooks = books.filter((book) => book.genres.find((g) => g === genre))
      //   return genredBooks
      // }
      // return books
      return books.filter((book) => {
        // if no author, automatic true, if author then the comparison decides if true or false. Same for genre.
